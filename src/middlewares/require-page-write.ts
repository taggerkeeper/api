...
const requirePageWrite = function (req: Request, res: Response, next: NextFunction): void {
  if (req.page?.revisions[0].permissions === undefined) {
    res.status(404).send({ message: 'Page not found.' })
  } else {
    const check = req.page?.revisions[0].permissions.canWrite(req.user, req.page?.revisions)
...
export default requirePageWrite
